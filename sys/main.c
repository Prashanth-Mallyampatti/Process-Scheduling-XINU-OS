...
int prA, prB, prC,prQ;
int proc_a(char), proc_b(char), proc_c(char),proc_q(char);
int proc(char);
volatile int a_cnt = 0;
volatile int b_cnt = 0;
volatile int c_cnt = 0;
volatile int q_cnt=0;
volatile int s = 0;
...
int main() {
	...
	if (s < 2) {
		...
		prQ=create(proc_q,2000,10,"proc Q","1",'Q');
		...
		kprintf("\nTest Result: A = %d, Q= %d, B = %d, C = %d\n", a_cnt,q_cnt, b_cnt, c_cnt);
	}
	// LINUXSCHED
	else {
...
int main()
{
printf("expdev:%d",(int)expdev(0.1));
setschedclass(1);
}*/
